...
def generate_density_plot(column, value): 
    ...
    plot = (pn.ggplot(appraisals, pn.aes(x = column)) + 
      ...
      pn.labs(title = f"Distribution of {value}", 
              x = "", y = "Density") +
      pn.theme_bw() +
...
def generate_box_plot(column, value):
    ...
    plot = (pn.ggplot(appraisals, pn.aes(x = column, y = "log_price")) + 
      ...
      pn.labs(title = f"Distribution of the Log of Price by {value}", 
              x = "", y = "") +
      pn.theme_bw() +   
      pn.theme(plot_title = pn.element_text(hjust = 0.5), 
               text = pn.element_text(size = 14))) 
...
generate_density_plot("price", "Price")
generate_density_plot("log_price", "the Log of Price")
...
(pn.ggplot(appraisals, pn.aes(x = "log_gba", y = "log_price")) + 
 ...
 pn.scale_y_continuous(labels = lambda x: [f"{i:g}" for i in x]) +
 pn.labs(title = "Log of Price vs. Log of Gross Building Area", 
